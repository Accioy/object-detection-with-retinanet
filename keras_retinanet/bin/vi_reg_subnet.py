...
for layer in layers:
    ...
    #     print(layer.name)
    layer1 = reg_submodel.get_layer(name='pyramid_regression_reshape')
    feature = K.function([model.layers[0].input], [L.output])
    P=feature([image])[0]
    regi = K.function([reg_submodel.layers[0].input],[layer1.output])
    f1=regi([P])[0][0]
    print(f1)
    # count=0
    # for x in f1:
    ...
    # plt.savefig(os.path.join("F:\\yan\\keras-retinanet-master\\visualization_results\\P_reg_lastc",layer+".jpg"))
