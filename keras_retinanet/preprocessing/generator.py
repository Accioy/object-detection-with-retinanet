...
class Generator(object):
    ...
    def filter_annotations(self, image_group, annotations_group, group):
        ...
        for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
            ...
            #对于： x1 < 0 | y1 < 0 | x2 <= 0 | y2 <= 0 | x2 >= image.shape[1] | y2 >= image.shape[0]，强制修改至图片范围内
            
            for x in range(len(annotations['bboxes'])):  
                temp=[]
                if((annotations['bboxes'][x, 0] < 0) | (annotations['bboxes'][x, 1] < 0) |
                    (annotations['bboxes'][x, 2] > image.shape[1]) | (annotations['bboxes'][x, 3] > image.shape[0])):
                    temp=annotations['bboxes'][x].copy()
                    if annotations['bboxes'][x, 0] < 0:
                        annotations['bboxes'][x, 0]=0
                    if annotations['bboxes'][x, 1] < 0:
                        annotations['bboxes'][x, 1] =0
                    if annotations['bboxes'][x, 2] > image.shape[1]:
                        annotations['bboxes'][x, 2] = image.shape[1]
                    if annotations['bboxes'][x, 3] > image.shape[0]:
                        annotations['bboxes'][x, 3] = image.shape[0]
                    print("Box change from ",temp," to ",annotations['bboxes'][x],"\n")
